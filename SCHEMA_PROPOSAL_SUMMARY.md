...
- `org_chart_oversight_record_id` (RecordID format) - Links to overseeing entity via RecordID (e.g., `100123`). **Replaces** `reports_to` field for org chart/political oversight relationships
- `org_chart_oversight_name` (text) - Name of the overseeing entity (derived from `org_chart_oversight_record_id`)
- `parent_organization_record_id` (RecordID format) - Links to parent entity via RecordID (e.g., `100318`). Used for specialized boards → parent, divisions → parent department, subdivisions → parent
...
- Linked to parent via `parent_organization_record_id` (RecordID, e.g., `100318`)
...
| `parent_organization_record_id` | 100382 |
| `parent_organization_name` | Office of Technology and Innovation |
| `org_chart_oversight_record_id` | 100382 |
...
| `organization_type` | Public Benefit or Development Organization |
| `governance_structure` | NYC Health + Hospitals is governed by a Board of Directors. Also has a Personnel Review Board (see 100212) for personnel matters. |
...
| `parent_organization_record_id` | 100318 |
...
- Links to overseeing entity using RecordID format (e.g., `100123`)
...
- Links to parent entity using RecordID format (e.g., `100318`)
